...
import homeImage from '../../assets/images/homepage_image.jpg';
import { FcExpand, FcSearch } from 'react-icons/fc';
import { MdExpandMore } from 'react-icons/md';
import { RxReset } from 'react-icons/rx';
import { useEffect, useState } from 'react';
import axios from 'axios';
import { CAPSULES_URL } from '../../utilities/config';
import './index.css';
...
const Home = () => {
  ...
  useEffect(() => {
    const queryUrl = `${CAPSULES_URL}?status=${statusQuery}&original_launch=${originalLaunchQuery}&type=${typeQuery}`;
    axios
      .get(queryUrl)
      .then((res) => {
        const typeData = [];
            ...
            <div id="resultsSection" className="card-container" style={{ padding: '5% 0' }}>
              ...
              <ul
                ...
                {data.map((eachData, index) => (
                  <RocketCard data={eachData} key={index} />
                ))}
              </ul>
            </div>
          </div>
